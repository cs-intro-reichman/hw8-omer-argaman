 ...
 public class User {
    ...
    public boolean addFollowee(String name) {
        if (this.fCount < maxfCount) {
            if (this.follows(name) || this.name.equalsIgnoreCase(name)) {
                return false; // already following the user or trying to follow self
            }
    ...
    public boolean removeFollowee(String name) {
        for (int i = 0; i < this.fCount; i++) {
            if (this.follows[i].equals(name)) {
                ...
                this.follows[this.fCount - 1] = null;  // remove the last element to make the array smaller
                this.fCount--;
                return true;
            }
        }
